...
def runScript(update_progress=None,GUI = None):
    ...
    updateGUI(10,"------------------------ Fine Tuning ------------------------",GUI,update_progress)
    np.random.seed(1)
    torch.manual_seed(1)
    ...
    if 1:
        ...
        updateGUI(0,
                  "************ Start ************\n\n" + 'GrapBert, dataset: ' + dataset_name + ', residual: ' + residual_type + ', k: ' + str(k) + ', hidden dimension: ' + str(hidden_size) +', hidden layer: ' + str(num_hidden_layers) + ', attention head: ' + str(num_attention_heads),
                  GUI, update_progress)
        ...
        print('************ Finish ************')
    updateGUI(10, "************ Finish ************", GUI, update_progress)
    #------------------------------------
        #add load net
    ...
    if 0:
        ...
        print('************ Start ************')
        print('GrapBert, dataset: ' + dataset_name + ', residual: ' + residual_type + ', k: ' + str(k) + ', hidden dimension: ' + str(hidden_size) +', hidden layer: ' + str(num_hidden_layers) + ', attention head: ' + str(num_attention_heads))
        updateGUI(10, "************ Start ************\n\n"+ 'GrapBert, dataset: ' + dataset_name + ', residual: ' + residual_type + ', k: ' + str(k) + ', hidden dimension: ' + str(hidden_size) +', hidden layer: ' + str(num_hidden_layers) + ', attention head: ' + str(num_attention_heads), GUI, update_progress)
        # ---- objection initialization setction ---------------
        data_obj = DatasetLoader()
        data_obj.dataset_source_folder_path = './data/' + dataset_name + '/'
        data_obj.dataset_name = dataset_name
