...
def runScript(numOfFile):
    ...
    def load_and_change_cluster2(clusters_net_t):
        ...
        ax.set(title="Confusion Matrix " +  str(numOfFile) , xlabel="Predict", ylabel="True")
        # ax.get_figure().show()
        ax.get_figure().savefig("./ImageResults/Matrix" + str(numOfFile) + ".png")
        np.savetxt(data_sve_temp + "file" + str(numOfFile), np.array(new_labels, dtype=int), fmt="%s")
    ...
    plt.savefig("./ImageResults/Model3D" + str(numOfFile) + ".png")
    # plt.show()
